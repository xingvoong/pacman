...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        self.beliefs.normalize()
        #self.beliefs = self.allPositions
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.allPositions:
            nDist = self.getPositionDistribution(gameState, pos)
            for np, p in nDist.items():
                dist[np] += self.beliefs[pos] * p
        #update
        self.beliefs = dist
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for par in self.particles:
            p = self.getObservationProb(observation, pacmanPosition, par, jailPosition)
            dist[par] += p

        dist.normalize()

        if dist.total() != 0: # the edge case
            self.beliefs = dist
...
class JointParticleFilter(ParticleFilter):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                if ghostDist is not None:
                    p *= self.getObservationProb(ghostDist, pacmanPosition, particle[i], jailPos)
                else:
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                prev = oldParticle
                newPosDist = self.getPositionDistribution(gameState, prev, i, self.ghostAgents[i])
